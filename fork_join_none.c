...
Student Name: Logan Rose

Student Number: 300059034
...
{
  int pid1, pid2, pid3, pid, parent;
  int status;
  int time_period[4] = {5,6,7,10};
  char str[128];


  //Gets the time lengths for each process as passed in by the command line arugments  
  ...
  pid1 = fork();
  //Fork returns negative if the process failed, exit condition on error, 
  if (pid1 < 0) {
